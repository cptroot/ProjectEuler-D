...
//Sieve of Eratosthenes, includes limit (limit is even);
bool[] Sieve(int limit) {
  float sqrtLimit = sqrt(limit);
  // true denotes composite number
  bool[] sieve = new bool[limit / 2 - 1];
  int[] edit = [];
  for (auto index = 3; index < sqrtLimit; index += 2) {
    if (index % 1000 == 1) writeln(index);
    if (!sieve[(index - 3) / 2]) {
      for (auto i = index; i <= limit / index; i += 2) {
        if (!sieve[(i - 3) / 2])
          edit ~= [(i * index - 3) / 2];
      }
      foreach (comp; edit)
